...
interface ContractEntry {
  ...
  /** When true, hooks accept contractAddress as a parameter instead of using static CONTRACTS config. */
...
function generateHooksFile(contract: ContractEntry, methods: QueryMethod[], allTypes: string[]): string {
  ...
  const dynamic = !!contract.dynamicAddress;
  ...
  if (dynamic) {
    ...
    lines.push(`  }`);
    lines.push(`  return queryContractSmart<T>(contractAddress, queryMsg);`);
    lines.push(`}`);
  } else {
    lines.push(`async function indexerWithFallback<T>(`);
    lines.push(`  queryMsg: Record<string, unknown>,`);
  ...
  for (const method of methods) {
    ...
    // Dynamic address contracts always need an enabled guard for the address
    const needsEnabled = requiredParams.length > 0 || dynamic;

    // Build function signature
    if (dynamic && hasParams) {
      const paramTypeParts = method.params.map(p =>
        `${p.name}${p.optional ? '?' : ''}: ${p.type}`
      );
      lines.push(`export function ${hookName}(contractAddress: string | undefined, params: { ${paramTypeParts.join('; ')} }) {`);
    } else if (dynamic) {
      lines.push(`export function ${hookName}(contractAddress: string | undefined) {`);
    } else if (hasParams) {
      const paramTypeParts = method.params.map(p =>
        `${p.name}${p.optional ? '?' : ''}: ${p.type}`
      );
    ...
    // Query key — include contractAddress for dynamic contracts
    const keyParts = [`KEY`, `'${method.queryMsgKey}'`];
    if (dynamic) keyParts.splice(1, 0, 'contractAddress');
    if (hasParams) {
    ...
    if (needsEnabled) {
      ...
      if (dynamic) conditions.push('!!contractAddress');
      conditions.push(...requiredParams.map(p => `params.${p.name} !== undefined`));
