---
title: Trustlessness
description: The Trustlessness Manifesto, verification pipelines, certified deployments, and how Terp Network minimizes trust assumptions
---

# Trustlessness

Trustlessness is the property of a system where participants do not need to rely on any single entity acting honestly. Instead, verification is built into the protocol itself — users can independently verify every claim the system makes.

## The Trustlessness Manifesto

Terp Network operates under a trustlessness mandate:

> **No user should be required to trust a third party to verify the integrity of the network, its software, or its state.**

This means:
- Every software release must be **reproducible** — anyone can build the same binary from source
- Every state transition must be **verifiable** — run a node and see for yourself
- Every "official" deployed contract must be **auditable** — source code is available and verifiable on-chain
- Every network upgrade must be **transparent** — governance procedures and upgrade proposals are public

> **Manifesto signature scaffold** — wallet integration for users to sign the trustlessness manifesto on-chain. The signed attestation serves as a commitment to trust-minimized principles. Implementation: a CosmWasm contract that records signatures + a minimal frontend component.

## Verification Pipelines

### Reproducible Builds

Terp Network uses deterministic build environments (Docker-based) to ensure that the compiled binary from source code matches the published release binary.

```bash
# Verify a release build yourself
git checkout v5.2.0
make build  # produces terpd binary
sha256sum build/terpd
# Compare with published checksum
```

> **Hash comparison visualizer scaffold** — an interactive component that accepts a file hash and visually compares it against published release checksums. See the [checksums discussion](/docs/resources/verify).

### Release Curation Pipeline

Terp Network follows a multi-stage pipeline for releases:

1. **Development** — PRs merged to main branch
2. **Testnet deployment** — O-Line deploys ephemeral nodes running the candidate
3. **Integration tests** — CI runs full test suite against testnet
4. **Governance vote** — if changes require parameter or state migrations
5. **Tagged release** — signed git tag + Docker image push
6. **Verification** — published checksums + reproducible build artifacts

> **Pipeline flow scaffold** — a flowchart showing the release curation pipeline from development through governance to release. Each stage can link to the relevant CI configuration or governance proposal.

### Certified Deployments

Smart contracts deployed via certain tools (O-Line, cw-orchestrator using Terp test infrastructure) can produce a **verification proof** that the on-chain code hash matches a known source build.

The verification flow:
```
Source code → compile + optimize → code_hash
                                        ↓
Deploy tx → on-chain code_id + code_hash
                                        ↓
Verify: does chain's code_hash match your local build?
```

## Trust Minimization in Practice

| Surface | Trust Assumption | How Terp Minimizes It |
|---------|-----------------|----------------------|
| Node software | Binary is correct | Reproducible builds + checksums |
| Chain state | State is valid | Run your own full node |
| Smart contract | Code does what it says | Source verification + on-chain code hash |
| IBC packet | Packet was delivered correctly | Light client verification |
| Governance | Proposal passes legitimately | On-chain vote tally, verifiable |
| Deployment | Deployed artifact matches source | CI pipeline + hash comparison |

## Further Reading

- [Verification guide](/docs/resources/verify) — verify binaries and deployments step-by-step
- [Build pipelines](/docs/guides/trustlessness) — deep dive into the automation
- [Integrity resources](/docs/resources/integrity) — checksums, signatures, attestations
