...
Once terpd is [installed](/connect/join) and configured with the correct network, you can now send commands with it. In a new terminal window, start by running the following query command:
...
```
In this example when we install terpd as a client with the [installer](../terp-core/terpd), it connects to the `http://rpc-terp.zenchainlabs.io:26657`. 
...
For testing and development purposes there is an `enabled-unsafe-cors` field inside [`app.toml`](/connect/join#configuring-the-node-using-apptoml).
