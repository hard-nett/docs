...
- Gas efficiency: on-chain ZK verification must be affordable for authentication use cases (target: <200k gas per verification)
- Security: proof verification must be sound — a forged proof must never pass on-chain verification
- Developer ergonomics: circuit authors must be able to define halo2 circuits, generate proving/verifying keys, and upload both in a single transaction
...
- Gas benchmark: verify that single-instance public input hashing achieves <200k gas for verification
