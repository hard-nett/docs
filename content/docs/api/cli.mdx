...
```json
{
	...
	"keyring-backend": "os",
	"output": "text",
	"node": "http://rpc.terp.network:26657",
	"broadcast-mode": "sync",
	"grpc-concurrency": false
}
```
In this example when we install terpd as a client with the [installer](/docs/), it connects to the `http://rpc.terp.network:443`. 

### Change node

```
Terp Network config node https://api.terp.network:443/
```

### Connect to the testnet

```bash
terpd config node https://api.terp.network:443/
