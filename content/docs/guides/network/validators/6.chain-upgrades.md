...
In this guide, we will go through upgrading your node during network upgrades, chain halt upgrades and hard forks. Depending on if you are using [Cosmovisor](/docs/) or not, there are key differences. Please note that the use of cosmovisor is recommended.
...
or look at upcoming upgrade proposals via the [block explorer](/docs/). It is recommended to be aware about the exact block height for the upgrade, so that action can be taken in case specific steps are required.
...
- Monitor the upgrade details on [Discord](https://discord.gg/9mFZc4XEDA) or stay updated about upcoming upgrade proposals via the [block explorer](/docs/). It is recommended to be aware about the exact block height for the upgrade, so that action can be taken in case specific steps are required.
...
```bash
cd "${HOME}"/.terpd/cosmovisor/upgrades/4.2.2/bin
```

Download the latest release, in this case:

```bash
wget - https://github.com/terpnetwork/terp-core/releases/download/v5.1.0/terpd_v5.1.0_linux_amd64
```
...
```bash
wget -O terpd_sha256.txt https://github.com/terpnetwork/terp-core/releases/download/v5.1.0/terpd_v5.1.0_checksums.txt
```
...
First, download the new genesis file to your local machine. The genesis file for each respective network can be found on the [networks](/docs/) page.
...
```bash
cp /path/to/new-genesis.json ~/.terpd/config/genesis.json
...
```bash
cd "${HOME}"/.terpd/cosmovisor/upgrades/v2.0.0/bin
```

And download the new terp-core binary from the <a href="https://github.com/terpnetwork/terp-core/releases" target="_blank">release page</a>.
...
```
wget https://github.com/terpnetwork/terp-core/releases/download/v5.1.0/terpd_v5.1.0_linux_arm64
```
