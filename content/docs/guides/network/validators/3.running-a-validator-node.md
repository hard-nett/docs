...
The first step is to run a node. See the [Node Installation](/connect/join) and [Joining a Network](/connect/join) guides for instructions on how to get your node up and running.

Make sure to [initialize your node](/connect/join) and [add your keys](../nodes/keys).
...
After having followed the [instructions to join a network](/connect/join) and your running node is synced, you can create a validator by executing the `create-validator` command. Please make sure to create a validator only once the node is fully synced, so that you avoid being slashed.
