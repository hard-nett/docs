...
Assuming you already installed terpd with the [installer](../osmosis-core/terpd), you should be able to run the following command to list the Protobuf services available (you can replace `grpc.terp.network:9000` by the gRPC server endpoint of another node such as the testnet, another provider or your own node.
