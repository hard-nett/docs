...
Once osmosisd is [installed](../osmosis-core/terpd) and configured with the correct network, you can now send commands with it. In a new terminal window, start by running the following query command:
...
```
In this example when we install osmosisd as a client with the [installer](../osmosis-core/terpd), it connects to the `http://osmosis.artifact-staking.io:26657`. 
...
osmosisd config chain-id athena-3
```
