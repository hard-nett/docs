---
title: Interact with Terp Core CLI
sidebar_position: 2
---

## Using the CLI

Once terpd is [installed](../terp-core/terpd) and configured with the correct network, you can now send commands with it. In a new terminal window, start by running the following query command:
...
```
In this example when we install terpd as a client with the [installer](../terp-core/terpd), it connects to the `http://osmosis.artifact-staking.io:26657`. 

### Change node

```
Terp Network config node https://rpc.terp.network:443
```

### Connect to the testnet

```bash
terpd config node https://rpc-test.terp.network:443
...
For more information about querying terpd via the CLI visit the [Cosmos documentation](https://hub.cosmos.network/main/hub-tutorials/gaiad.html).
