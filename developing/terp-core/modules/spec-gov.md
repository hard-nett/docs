...
The ```gov``` module enables on-chain governance which allows TerpNET token holders to participate in a community led decision-making process. For example, users can:
...
- **```deposit_params```** - Deposit related parameters
  - **```min_deposit```**: Minimum deposit (in uTERP) for a proposal to enter voting period
...
```bash
terpd tx gov submit-proposal --title="Match External Incentives for DOGE/OSMO and DOGE/ATOM pairs" --description="Input description" --type="Text" --from=WALLET_NAME --chain-id=CHAIN_ID
```
:::
...
```bash
terpd tx gov submit-proposal param-change proposal.json --from WALLET_NAME --chain-id CHAIN_ID
```
...
```
terpd tx gov submit-proposal community-pool-spend proposal.json --from WALLET_NAME --chain-id CHAIN_ID
```
...
Update terp-core to V!:

```bash
terpd tx gov submit-proposal software-upgrade proposal.json --from WALLET_NAME --chain-id CHAIN_ID
```
...
```json
{
  ...
  "height": "1314500",
  "info": "https://raw.githubusercontent.com/terpnetwork/networks/main/terp-core/upgrades/v1/mainnet/upgrade_4_binaries.json",
},
```
:::
...
```bash
terpd tx gov submit-proposal cancel-software-upgrade --title="cancel v4" --description="cancel v4 upgrade" --from=WALLET_NAME --chain-id=CHAIN_ID
```
:::
...
```bash
terpd tx gov deposit 12 500000000uterp --from WALLET_NAME --chain-id CHAIN_ID
```
:::
...
```bash
terpd tx gov vote 12 yes --from WALLET_NAME --chain-id CHAIN_ID
```
:::
...
```bash
terpd query gov proposals -o json | jq
```
...
```bash
terpd query gov proposal 13 -o json | jq
```
:::
...
```bash
terpd query gov tally 13 -o json | jq
```
...
```bash
terpd query gov params --output json | jq
```
